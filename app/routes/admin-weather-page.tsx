...
import type { Route } from "./+types/admin-weather-page";
...
import {
  ...
  DEFAULT_WEATHER_SEO,
  mergeWeatherContent,
} from "../lib/weather-page-config";

export function meta() {
  return [{ title: `Weather Page | Admin | ${siteConfig.siteName}` }];
}
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  const page = await getSystemPage("weather");
  return {
    content: mergeWeatherContent(page?.content),
    seo: {
      meta_title: page?.meta_title || DEFAULT_WEATHER_SEO.meta_title,
      meta_description: page?.meta_description || DEFAULT_WEATHER_SEO.meta_description,
      focus_keyphrase: page?.focus_keyphrase || DEFAULT_WEATHER_SEO.focus_keyphrase,
      og_image: page?.og_image || DEFAULT_WEATHER_SEO.og_image,
      slug: "weather",
    },
  };
}
...
export async function action({ request }: Route.ActionArgs) {
  ...
  let content: Record<string, any> = DEFAULT_WEATHER_PAGE;
  try {
    content = JSON.parse(raw);
  } catch {
    content = DEFAULT_WEATHER_PAGE;
  }
  ...
  return redirect("/admin/pages/weather?toast=Weather+page+saved");
}
...
export default function AdminWeatherPage() {
  ...
  const rootData = useRouteLoaderData("root") as
    ...
    | undefined;
  const [data, setData] = useState(content as typeof DEFAULT_WEATHER_PAGE);
  ...
  const handleSeoAiGenerate = async (field: "title" | "description") => {
    ...
    try {
      const res = await fetch("/api/ai-seo-meta", {
        ...
        body: JSON.stringify({
          field,
          name: hero.title || "Weather & Forecast",
          pageType: "page",
          slug: seo.slug,
    ...
    try {
      ...
      const res = await fetch("/api/ai-keyphrase", {
        ...
        body: JSON.stringify({
          bodyHtml,
          contentType: "page",
          title: hero.title || "Weather & Forecast",
          slug: seo.slug,
        }),
      });
      const result = await res.json();
  ...
  const renderedTitle = applyTitleTemplate(
    metaTitle || hero.title || "Weather & Forecast",
    titleTemplate
  );
  ...
  return (
    <div className="px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-dark">Weather Page Content</h1>
        <a
          href="/weather"
          target="_blank"
          rel="noopener"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  value={hero.height}
                  onChange={(event) =>
                    update("hero.height", Math.min(520, Math.max(220, Number(event.target.value) || 280)))
                  }
                  className={inputClass}
                />
              </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className={inputClass}
                placeholder="e.g. bryce canyon national park weather forecast"
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className={inputClass}
                placeholder="SEO title for /weather"
              />
            ...
            <div>
              ...
              <textarea
                ...
                className={inputClass}
                placeholder="SEO description for /weather"
              />
              <p className="text-xs text-gray-400 mt-1">
...
function HeroPreview({
  ...
  const from = hero.gradient_from || "#1e3a5f";
  const via = hero.gradient_via || "#2d4a6f";
  const to = hero.gradient_to || "#1a3050";
  const opacity =
  ...
  return (
    <div className="relative h-36 rounded-lg overflow-hidden border border-gray-200">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-xs mt-1" style={{ color: hero.subtitle_color || "#cbd5e1" }}>
          {hero.subtitle}
        </p>
      </div>
    </div>
