...
import type { Route } from "./+types/admin-directory-page";
...
import {
  mergeDirectoryPageContent,
  type DirectoryHeroContent,
} from "../lib/directory-pages";
import { getSystemPage, upsertSystemPage } from "../lib/pages.server";

export function meta({ data }: Route.MetaArgs) {
  const title = (data as any)?.directory?.title || "Directory Page";
  return [{ title: `${title} | Admin | ${siteConfig.siteName}` }];
}
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const config = getDirectoryConfig(slug);
  if (!config) {
    throw new Response("Directory page not found", { status: 404 });
  }

  const [page, settings] = await Promise.all([getSystemPage(slug), getSettings()]);
  const content = mergeDirectoryPageContent(slug, page?.content);
  ...
  return {
    ...
    content,
    seo: {
      meta_title: page?.meta_title || settings[`meta_title_${slug}`] || "",
      meta_description:
        page?.meta_description || settings[`meta_description_${slug}`] || "",
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  const content = mergeDirectoryPageContent(slug, parsed);
  ...
  await upsertSystemPage({
    slug,
    title: `${config.title} Directory Page`,
    content,
  ...
  return redirect(
    `/admin/pages/directory/${slug}?toast=${encodeURIComponent(
      `${config.title} page saved`
    )}`
  );
}

export default function AdminDirectoryPage() {
  const { directory, content, seo } = useLoaderData<typeof loader>();
  ...
  const handleSeoAiGenerate = async (field: "title" | "description") => {
    ...
    try {
      const res = await fetch("/api/ai-seo-meta", {
        ...
        body: JSON.stringify({
          field,
          name: hero.title || directory.title,
          pageType: "directory",
          slug: directory.slug,
          description: hero.subtitle || directory.subtitle,
    ...
    try {
      ...
      const res = await fetch("/api/ai-keyphrase", {
        ...
        body: JSON.stringify({
          bodyHtml,
          contentType: "page",
          title: hero.title || directory.title,
          slug: seo.slug,
        }),
      });
      const result = await res.json();
  ...
  const renderedTitle = applyTitleTemplate(
    metaTitle || hero.title || directory.title,
    titleTemplate
  );
    ...
    <div className="px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-dark">
          {directory.title} Directory Page
        </h1>
        <a
          href={directory.path}
          target="_blank"
        ...
        >
          <div className="space-y-4">
            ...
            <Field
              ...
              onChange={(value) => update("hero.search_placeholder", value)}
              inputClass={inputClass}
            />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    update(
                      "hero.height",
                      Math.min(520, Math.max(220, Number(event.target.value) || 300))
                    )
                  }
                  className={inputClass}
                />
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      update("hero.subtitle_color", event.target.value)
                    }
                    className="h-10 w-full p-1 border border-gray-300 rounded-lg bg-white"
                  />
                </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) =>
                    update("hero.gradient_opacity", Number(event.target.value))
                  }
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                Directory page route slug is fixed in routing.
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className={inputClass}
                placeholder={`e.g. zion ${directory.slug} guide`}
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className={inputClass}
                placeholder={`SEO title for /${directory.slug}`}
              />
            ...
            <div>
              ...
              <textarea
                ...
                className={inputClass}
                placeholder={`SEO description for /${directory.slug}`}
              />
              <p className="text-xs text-gray-400 mt-1">
...
function HeroPreview({ hero }: { hero: DirectoryHeroContent }) {
  const from = hero.gradient_from || "#111827";
  const via = hero.gradient_via || "#1f2937";
  const to = hero.gradient_to || "#92400e";
  const opacity =
    typeof hero.gradient_opacity === "number" ? hero.gradient_opacity : 85;
    ...
    <div className="relative h-36 rounded-lg overflow-hidden border border-gray-200">
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
        ...
        <p
          ...
          style={{ color: hero.subtitle_color || "#f3f4f6" }}
        >
          {hero.subtitle}
        </p>
      </div>
